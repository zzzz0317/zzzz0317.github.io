var allowedHost = ['blog.asec01.net','archive.org'];
function loadSwal() {
...
function showSwal() {
    swal({
        title: '注意',
        html: '您似乎从一个非授权的镜像站点(' + window.location.search.replace('?','') + ')访问本博客<br>我现在已经将您带向原站点~',
        type:'warning',
        timer: 5000,
...
queue.offer(function(){
    ...
    if (!allowedHost.includes(window.location.host)) {
        if (window.location.pathname === '/' && window.location.search === '') {
            window.location.href = 'https://blog.asec01.net/?' + window.location.host;
        }
        if (window.location.pathname === '/' && window.location.search.startsWith('?')) {
            window.location.href = 'https://blog.asec01.net/' + window.location.search.replace('?','') + '?' + window.location.host;
        }
        if (window.location.pathname != '/') {
            window.location.href = 'https://blog.asec01.net' + window.location.pathname + '?' + window.location.host;
        }
    }
    if(window.location.host === 'blog.asec01.net' && window.location.search !== '') {
        loadSwal();
    }
});